...
sub visTp_TafuBlA {
    my $G = shift;
    my ($Tp, $Bup, $A) = @_;
    my $old = $G->visTp($Tp); # could be Fun, wire into end
    my $uu = $G->Bu(@$Bup); 
    my $u = $G->fu($uu);
...
sub scGre {
    ...
    my @a = grep { $G->ip($ip, $_->{i}) } @{ $G->{W}->{script} };
    @a
}
...
sub T {
    ...
    $G->accum($T->{L}->{i}, 'Li', $T->{L}); # just right
