...
.'<span id="<<ID>>-name-'.$i.'" style="position: absolute;top: 182px; left: 3px;  display: block;z-index: 20;text-shadow: 0 3px 45px #fff;font-size: 130pt;color: rgba(0,0,0,0);">'.$H->encode_entities($c->{first}).'</span>'
        }
...
CM
    ...
    $H->JS("\$('.CodeMirror').css('background','#1F1F7A').css('width', '100%').css('height', 'auto').css('overflow', 'visible');");
